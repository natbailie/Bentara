...
export default function ReportPage() {
    ...
    const handleSignOff = async () => {
        ...
        try {
            /** * REPLACED: manual fetch with api.post
             */
            await api.post(`/reports/${params.id}/signoff`);
            fetchReport();
        } catch (err) {
            alert("Authorization Failed: Permission denied.");
        } finally {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-black"><Loader2 className="animate-spin text-slate-400" /></div>;
            ...
            <div className="max-w-4xl mx-auto bg-white p-12 min-h-[1123px] shadow-2xl print:shadow-none print:min-h-0 relative">
                ...
                <div className="flex justify-between items-start border-b-2 border-slate-900 pb-8 mb-8 relative z-10">
                    <div className="flex items-center gap-4">
                        <div className="w-20 h-20 bg-white rounded-lg border border-slate-100 p-1"><img src="/bentaralogo.jpg" className="w-full h-full object-contain" alt="Bentara Logo"/></div>
                ...
                <div className="mb-8 break-inside-avoid relative z-10">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        <div onClick={() => setIsExpanded(true)} className="relative w-72 h-72 bg-slate-100 rounded-lg border border-slate-200 overflow-hidden shrink-0 cursor-zoom-in hover:shadow-lg transition-all group">
                            {/* FIXED: Using getBaseUrl() to point to the cloud backend */}
                            <img src={`${getBaseUrl()}${data.image_url}`} className="w-full h-full object-cover" alt="Slide Scan" />
            ...
            {isExpanded && (
                <div className="fixed inset-0 z-50 bg-black/95 flex overflow-hidden">
                    ...
                    >
                        ...
                        >
                            {/* FIXED: Using getBaseUrl() to point to the cloud backend */}
                            <img src={`${getBaseUrl()}${data.image_url}`} draggable={false} className="max-w-none" style={{ height: '80vh' }} alt="High Res Scan" />
                            {data.detections && data.detections.map((box: any, i: number) => {
                                if (!activeSubtypes.has(box.label)) return null;
                                return (
                                    <div key={i} className="absolute border-2 border-red-500/80 bg-red-500/10"
                        ...
                        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-white px-6 py-3 rounded-full flex items-center gap-6 shadow-2xl border border-white/10">
                            <div className="flex items-center gap-2 border-r border-white/20 pr-4">
                                ...
                                <input type="text" value={manualZoom} onChange={handleManualZoomChange} onKeyDown={handleKeyDown} onBlur={applyManualZoom} className="w-12 bg-transparent text-center font-mono focus:outline-none focus:text-blue-400"/>
                            ...
                            <button onClick={() => setIsExpanded(false)} className="text-xs font-bold text-white/50 hover:text-white border-l border-white/20 pl-6 flex items-center gap-2"><X size={16}/> Close Viewer</button>
                        </div>
                    </div>
